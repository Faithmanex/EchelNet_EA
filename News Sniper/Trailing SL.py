...
TRAIL_AMOUNT = 0.25  # Amount by how much SL updates
 ...
 if positions:
     for position in positions:
         ...
         if position.symbol in SYMBOLS:
             ...
             dist_from_sl = abs(round((price_current - sl) * point, 6))

             if dist_from_sl > MAX_DIST_SL * point:
                # calculating new sl
                if sl != 0.0:
                  if order_type == 0: # 0 stands for BUY
                      new_sl = trail + TRAIL_AMOUNT * point
                      if sl > price_open:
                          trail = sl
                          new_sl = trail + TRAIL_AMOUNT * point
                  ...
                  new_sl = price_open - DEFAULT_SL * point if order_type == 0 else price_open + DEFAULT_SL * point
